...
- **Do NOT run `next build` while the user's `next dev` is up**: both write to the shared `.next/`, and the build clobbers the dev server's route manifests — every route-group page starts 404ing until the user restarts dev with `rm -rf .next && npm run dev` (bit us 2026-07-15). Ask the user to pause dev first, or skip build-based verification.
